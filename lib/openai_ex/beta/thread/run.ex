defmodule OpenaiEx.Beta.Thread.Run do
  ...
  alias OpenaiEx.Beta.Schema.{Tool, RunError, RunRequiredAction}
  ...
  # NOTE: can get assistant response after create run
  def get_assistant_reponse(thread_id, run_id) do
    run = retrieve(thread_id, run_id)

    case run.status == :completed do
      false ->
        get_assistant_reponse(run.thread_id, run.id)

      _ ->
        [message | _] = Message.list(run.thread_id)
        message
    end
  end
